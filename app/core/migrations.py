...
def seed_defaults(session: Session) -> None:
    ...
    _seed_people_and_transactions(session)
...
def _seed_people_and_transactions(session: Session) -> None:
    if session.scalar(select(Person).limit(1)) is None:
        session.add_all(
            [
                Person(name="Me", normalized_name="me", email="", is_house_member=True, is_creditor=True, is_debtor=True, note="Default owner"),
                Person(name="Ahmed", normalized_name="ahmed", is_house_member=True, is_creditor=True, is_debtor=True),
                Person(name="Youssef", normalized_name="youssef", is_house_member=True, is_creditor=True, is_debtor=True),
                Person(name="Omar", normalized_name="omar", is_house_member=True, is_creditor=True, is_debtor=True),
            ]
        )
        session.flush()
    ...
    if session.scalar(select(Transaction).limit(1)) is None:
        cats = {c.name: c for c in session.scalars(select(Category)).all()}
        today = date.today()
        session.add_all(
            [
                Transaction(type=TransactionType.INCOME, amount=Decimal("2500.00"), category=cats["Salary"], date=today.replace(day=1), note="Monthly salary"),
                Transaction(type=TransactionType.EXPENSE, amount=Decimal("64.30"), category=cats["Food"], date=today - timedelta(days=2), payment_method="Card", note="Groceries"),
                Transaction(type=TransactionType.EXPENSE, amount=Decimal("780.00"), category=cats["Rent"], date=today - timedelta(days=8), payment_method="Bank Transfer"),
                Transaction(type=TransactionType.SAVING, amount=Decimal("350.00"), category=cats["Savings"], date=today - timedelta(days=4), payment_method="Bank Transfer"),
                Transaction(type=TransactionType.EXPENSE, amount=Decimal("38.90"), category=cats["Transport"], date=today - timedelta(days=1), payment_method="Card"),
            ]
        )
